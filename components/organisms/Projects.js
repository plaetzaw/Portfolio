    ...
    return (
      <div key={index} className='px-4 py-8 flex flex-col justify-center md:flex-row'>
        <div className='md:w-1/3'>
          <div className='md: pr-4'><Image src={project.src} height={250} width={250} /></div>
          <div className='flex flex-row justify-center text-[#F2EFEF]'>
            <div onClick={() => { Navigate(project.website) }}><Website /><span>Deploy</span></div>
            <div onClick={() => { Navigate(project.feURL) }}><Github /><span>Front-End</span></div>
            <div onClick={() => { Navigate(project.beURL) }}><Github /><span>Back-End</span></div>
          </div>
        </div>
